...
export type OrbitGraphQueryInput = {
  userId: string;
  scope: OrbitGraphScope;
  nodeCap: number;
  expandAnchorIds: string[];
};
...
export async function buildOrbitGraphPayload(
  input: OrbitGraphQueryInput
): Promise<Omit<OrbitGraphPayload, "generatedAt">> {
  const { userId, scope, nodeCap, expandAnchorIds } = input;
  ...
  const [tagsRaw, collectionsRaw, totalBookmarks, totalLooseBookmarks, bookmarksInitial] =
    await Promise.all([
      prisma.tag.findMany({
        where: { userId },
        select: {
  ...
  let bookmarksRaw = bookmarksInitial;
  ...
  for (const bookmark of bookmarksRaw) {
    ...
    for (const { tagId } of bookmark.tags) {
      ...
      tagBookmarkCounts.set(tagId, (tagBookmarkCounts.get(tagId) ?? 0) + 1);
    }
  ...
  for (const collection of collectionsRaw) {
    const variant =
      collection.type === "x_folder" ? "x_folder" : "user_collection";
    if (variant === "x_folder") {
      xFolderCount += 1;
    } else {
