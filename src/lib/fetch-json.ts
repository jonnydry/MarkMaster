...
export async function fetchJson<T>(
  ...
  if (!res.ok) {
    throw fetchJsonErrorFor(res, body);
  }
...
function fetchJsonErrorFor(res: Response, body: unknown) {
  let message =
  ...
  return new FetchJsonError(message, res.status, body);
}

/** The error `fetchJson` would throw for a failed response (streaming callers). */
export async function readFetchJsonError(res: Response) {
  return fetchJsonErrorFor(res, await readResponseBody(res));
}
