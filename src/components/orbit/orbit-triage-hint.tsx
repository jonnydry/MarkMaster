  ...
  return (
    <div
      className={cn(
        ...
        isOrbital ? "border-primary/15 bg-primary/5 text-muted-foreground" : "border-white/10 bg-white/[0.03] text-white/65",
        className
      )}
    >
      <button
        ...
        className={cn(
          ...
          isOrbital ? "text-primary/50 hover:text-primary" : "text-white/40 hover:text-white/70"
        )}
      ...
      <ul className={cn("mt-1.5 list-inside list-disc space-y-0.5 pr-4", orbitMetaMuted(isOrbital))}>
        <li>
          <strong className={isOrbital ? "text-foreground/90" : "text-white/80"}>
            Scan
          </strong>{" "}
